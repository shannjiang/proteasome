...
#prot_dat <- filter(prot_dat, Reverse != "+", Potential.contaminant != "+")
...
#prot_LFQ_dat = prot_dat[,prot_LFQ_columns]
#prot_LFQ_dat = prot_LFQ_dat[rowSums(prot_LFQ_dat[])>0,]
#prot_dat = prot_dat[rownames(prot_LFQ_dat),]
...
experimental_design$condition = rep(c('AD','CTR'),c(9,10))
experimental_design$replicate = c(c(1:9),c(1:10))
...
# if plot_cor error, change alpha threshold
dep <- add_rejections(data_diff, alpha = 0.75, lfc = log2(1.5))

#################visualization of the results
# Plot the first and second principal components (PCA)
plot_pca(dep, x = 1, y = 2, indicate = c("condition"), n = 500, point_size = 4)
...
#             k = 2, col_limit = 10, show_row_names = FALSE)
...
col_annot_df = data.frame(condition = rep(c('CTR','AD'),c(10,9)))
rownames(col_annot_df) = c(paste0('CTR_',c(1:10)),paste0('AD_',c(1:9)))
...
save_pheatmap_pdf(fig, paste0(work_dir,'WM_purified_proteomics_proteasome_gene_heatmap.pdf'), width = 3.7, height = 7)
...
fig = pheatmap(datExpr2,color = colorRampPalette(rev(brewer.pal(n = 7, name =
  "RdBu")))(100), cellheight = 12, cellwidth = 12, scale = 'row', cluster_rows = F, cluster_cols = F, show_rownames = T, show_colnames = F, fontsize_row = 14,annotation_colors = annot_colors, annotation_col = col_annot_df, annotation_row = row_annot_df)
save_pheatmap_pdf(fig, paste0(work_dir,'purified_proteomics_WM_proteasome_gene_heatmap.pdf'), width = 8, height = 10)
...
pdf(paste0(work_dir,'purified_proteomics_WM_all_gene_volcano.pdf'),width = 3.3, height = 3.3)
ggplot(data_imp_res,aes(x=AD_vs_CTR_ratio,y=log10P)) + geom_point(aes(color=volcano_color),size=0.5,alpha=0.5) + scale_color_manual(breaks = unique(data_imp_res$volcano_color), values = volcano_colr) + theme(legend.position = "none") + xlab('Log2(fold change)') + ylab('-Log10(P value)') + geom_hline(yintercept=-log10(0.05),linetype='dashed',color='black') + geom_text(data=subset(data_imp_res,annot_thre %in% c('TRUE')),aes(label=name),size=3,nudge_x = 0,nudge_y = 0.2,check_overlap = TRUE) + xlim(-3,4)
dev.off()
...
pdf(paste0(work_dir,'purified_proteomics_WM_proteasome_gene_volcano.pdf'),width = 3.3, height = 3.3)
ggplot(data_imp_res,aes(x=AD_vs_CTR_ratio,y=log10P)) + geom_point(aes(color=volcano_color),size=0.5,alpha=0.5)+ geom_point(data=subset(data_imp_res,volcano_color %in% c('blue','red')),aes(color=volcano_color),size=1.5,alpha=0.5) + scale_color_manual(breaks = unique(data_imp_res$volcano_color), values = volcano_colr) + theme(legend.position = "none") + xlab('Log2(fold change)') + ylab('-Log10(P value)') + geom_hline(yintercept=-log10(0.05),linetype='dashed',color='black') + geom_vline(xintercept=-0.5,linetype='dashed',color='black') + geom_vline(xintercept=0.5,linetype='dashed',color='black') + geom_text(data=subset(data_imp_res,volcano_color %in% c('blue','red')),aes(label=name),size=3,nudge_x = 0,nudge_y = 0.2,check_overlap = TRUE)
dev.off()
