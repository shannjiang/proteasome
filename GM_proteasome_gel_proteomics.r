...
prot_dat = read.csv(file = paste0(work_dir,'GM_proteinGroups.csv'), header = T)
...
experimental_design$condition = rep(c('AD','CTR','AD','CTR','AD'),c(4,4,4,8,6))
experimental_design$replicate = c(c(1:4),c(1:4),c(5:8),c(5:12),c(9:14))
...
write.csv(data_imp_res, file = paste0(work_dir,'GM_protein_data_imp_res.csv'))

############## all gene heatmap
datExpr = as.data.frame(data_imp@assays@data)
datExpr = datExpr[,!colnames(datExpr) %in% c('group','group_name')]
col_annot_df = data.frame(condition = rep(c('CTR','AD'),c(12,14)))
...
save_pheatmap_pdf(fig, paste0(work_dir,'GM_gel_proteomics_proteasome_gene_heatmap.pdf'), width = 3.7, height = 7)
...
fig = pheatmap(datExpr2, color = colorRampPalette(rev(brewer.pal(n = 7, name =
  "RdBu")))(100), cellheight = 8, cellwidth = 8, scale = 'row', cluster_rows = F, cluster_cols = F, show_rownames = T, show_colnames = F, fontsize_row = 8,annotation_colors = annot_colors, annotation_col = col_annot_df, annotation_row = row_annot_df)
save_pheatmap_pdf(fig, paste0(work_dir,'gel_proteomics_GM_proteasome_gene_heatmap.pdf'), width = 7, height = 7)
...
data_imp_res$annot_thre = with(data_imp_res,ifelse(-log10(AD_vs_CTR_p.val) > 2.75, 'TRUE', 'FALSE'))
#options(ggrepel.max.overlaps = Inf)
pdf(paste0(work_dir,'gel_proteomics_GM_all_gene_volcano.pdf'),width = 3.3, height = 3.3)
#ggplot(data_imp_res,aes(x=AD_vs_CTR_ratio,y=log10P)) + geom_point(aes(color=volcano_color),size=1.5,alpha=0.5) + scale_color_manual(breaks = unique(data_imp_res$volcano_color), values = volcano_colr) + theme(legend.position = "none") + xlab('Log2(fold change)') + ylab('-Log10(P value)') + geom_hline(yintercept=-log10(0.05),linetype='dashed',color='black') + geom_text_repel(data=subset(data_imp_res,annot_thre %in% c('TRUE')),aes(label=name),size=3)
ggplot(data_imp_res,aes(x=AD_vs_CTR_ratio,y=log10P)) + geom_point(aes(color=volcano_color),size=0.5,alpha=0.5) + scale_color_manual(breaks = unique(data_imp_res$volcano_color), values = volcano_colr) + theme(axis.text = element_text(size = 12),axis.title=element_text(size=12),legend.position = "none") + xlab('Log2(fold change)') + ylab('-Log10(P value)') + geom_hline(yintercept=-log10(0.05),linetype='dashed',color='black') + geom_text(data=subset(data_imp_res,annot_thre %in% c('TRUE')),aes(label=name),size=3,nudge_x = 0,nudge_y = 0.2,check_overlap = TRUE) + xlim(-3.5,3.5)
dev.off()
...
pdf(paste0(work_dir,'gel_proteomics_GM_proteasome_gene_volcano.pdf'),width = 3.3, height = 3.3)
#ggplot(data_imp_res,aes(x=AD_vs_CTR_ratio,y=log10P)) + geom_point(aes(color=volcano_color),size=1.5,alpha=0.5) + geom_point(data=subset(data_imp_res,volcano_color %in% c('blue','red')),aes(color=volcano_color),size=1.5,alpha=0.5) + scale_color_manual(breaks = unique(data_imp_res$volcano_color), values = volcano_colr) + theme(legend.position = "none") + xlab('Log2(fold change)') + ylab('-Log10(P value)') + geom_hline(yintercept=-log10(0.05),linetype='dashed',color='black') + geom_vline(xintercept=-0.5,linetype='dashed',color='black') + geom_vline(xintercept=0.5,linetype='dashed',color='black') + geom_text_repel(data=subset(data_imp_res,volcano_color %in% c('blue','red')),aes(label=name),size=3)
