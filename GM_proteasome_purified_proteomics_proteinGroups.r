...
experimental_design$condition = rep(c('AD','CTR'),c(10,10))
experimental_design$replicate = rep(c(1:10),2)
...
write.csv(data_imp_res, file = paste0(work_dir,'protein_data_imp_res.csv'))

############## all gene heatmap
datExpr = as.data.frame(data_imp@assays@data)
datExpr = datExpr[,!colnames(datExpr) %in% c('group','group_name')]
col_annot_df = data.frame(condition = rep(c('CTR','AD'),c(10,10)))
rownames(col_annot_df) = c(paste0('CTR_',c(1:10)),paste0('AD_',c(1:10)))
...
save_pheatmap_pdf(fig, paste0(work_dir,'GM_purified_proteomics_all_gene_heatmap.pdf'), width = 3.7, height = 7)
...
fig = pheatmap(datExpr2,color = colorRampPalette(rev(brewer.pal(n = 7, name =
  "RdBu")))(100), cellheight = 11, cellwidth = 11, scale = 'row', cluster_rows = F, cluster_cols = F, show_rownames = T, show_colnames = F, fontsize_row = 13,annotation_colors = annot_colors, annotation_col = col_annot_df, annotation_row = row_annot_df)
save_pheatmap_pdf(fig, paste0(work_dir,'purified_proteomics_GM_proteasome_gene_heatmap.pdf'), width = 8, height = 10)
...
pdf(paste0(work_dir,'purified_proteomics_GM_all_gene_volcano.pdf'),width = 3.3, height = 3.3)
ggplot(data_imp_res,aes(x=AD_vs_CTR_ratio,y=log10P)) + geom_point(aes(color=volcano_color),size=0.5,alpha=0.5) + scale_color_manual(breaks = unique(data_imp_res$volcano_color), values = volcano_colr) + theme(legend.position = "none") + xlab('Log2(fold change)') + ylab('-Log10(P value)') + geom_hline(yintercept=-log10(0.05),linetype='dashed',color='black') + geom_text(data=subset(data_imp_res,annot_thre %in% c('TRUE')),aes(label=name),size=3,nudge_x = 0,nudge_y = 0.2,check_overlap = TRUE) + xlim(-3,6.5)
dev.off()
...
pdf(paste0(work_dir,'purified_proteomics_GM_proteasome_gene_volcano.pdf'),width = 3.3, height = 3.3)
